...
# Add the parent directory to sys.path to allow imports from the parent directory (aiie)
...
with st.expander("⏳ Timelines ✨", expanded=True):
    ...
    data = df[C.occurred].value_counts().rename_axis('Year').reset_index(name='Incidents')
...
merge_rename_operations_technology = {
    ...
    'tech_facial recognition/detection/identification ': ['tech_facial recognition', 'tech_facial detection',
                                                          'tech_facial recogniton', 'tech_facial detection'],
    ...
    'tech_speech/voice recognition ': ['tech_speech/voice recognition', 'tech_speech ecognition',
                                       'tech_speech recognition', 'tech_voice recognition'],
    ...
    'tech_object recognition/detection/identification ': ['tech_object identification', 'tech_object recognition',
    ...
    'tech_behavioural analysis ': ['tech_behavioural monitoring', 'tech_behavioural monitoring system',
    ...
    'tech_fingerprint analysis ': ['tech_fingerprint biometrics', 'tech_fingerprint scanning',
                                   'tech_fingerprint recognition'],
    'tech_risk assessment algorithm/system ': ['tech_risk assessment algorithm',
...
merge_rename_operations_sector = {
    ...
    'sector_business/professional services ': ['sector_professional/business services',
...
merge_rename_operations_issue = {
    'issue_bias/discrimination - lgbtqi+': ['issue_bias/discrimination - transgender', 'issue_transgender',
                                            'issue_bias/discrimination - sexual preference (lgbtq)',
                                            'issue_bias/discrimination - lgbtq', 'issue_lgbtq', ],
    'issue_necessity/proportionality ': ['issue_necessity/proportionality', 'issue_proportionality'],
    'issue_bias/discrimination - race/ethnicity ': ['issue_bias/discimination - race', 'issue_race',
                                                    'issue_bias/disrimination - race', 'issue_ethnicity',
    ...
    'issue_bias/discrimination - political ': ['issue_bias/discrimination - politics',
                                               'issue_bias/discrimination - political', 'issue_political'],
    ...
    'issue_governance/accountability - capability/capacity ': ['issue_capability/capacity',
    ...
    'issue_bias/discrimination - employment/income ': ['issue_bias/discrimination - employment', 'issue_employment',
                                                       'issue_bias/discrimination - income', 'issue_income',
    ...
    'issue_bias/discrimination - political opinion/persuasion ': ['issue_bias/discrimination - political opinion',
    ...
    'issue_employment - pay/compensation ': ['issue_pay', 'issue_employment - pay',
    ...
    'issue_accuracy/reliabiity ': ['issue_accuracy/reliabiity', 'issue_accuracy/reliability',
                                   'issue_accuracy/reliabilty', 'issue_accuracy/reliablity',
    ...
    'issue_humanrights_freedom': ['issue_freedom of expression - right of assembly',
                                  'issue_freedom of expression - censorship', 'issue_freedom of expression',
...
merge_rename_operations_transparency = {
    ...
    'transp_marketing ': ['transp_marketing: privacy', 'transp_marketing privacy', 'transp_marketing',
                          'transp_governance: marketing', 'transp_marketing - hype', 'transp_marketing - misleading'],
    'transp_privacy/consent': ['transp_privacy - consent', 'transp_consent', 'transp_privacy'],
    'transp_complaints/appeals': ['transp_complaints/appeals', 'transp_complaints & appeals',
...
with st.expander("🔥 Interactive Heatmaps for Feature Relationships ✨", expanded=True):
    st.markdown("#### Explore the relationships between different features through interactive heatmaps.")
    ...
    df_processed, transparency_set = preprocess_column(df_processed, 'Transparency', 'transp',
                                                       merge_rename_operations_transparency)

    # Extracting technology and issue columns with dynamic top N filtering
    tech_columns = get_top_columns(df_processed, [col for col in df_processed.columns if col.startswith('tech_')],
                                   top_n)
    issue_columns = get_top_columns(df_processed, [col for col in df_processed.columns if col.startswith('issue_')],
                                    top_n)
    sector_columns = get_top_columns(df_processed, [col for col in df_processed.columns if col.startswith('sector_')],
                                     top_n)
    transp_columns = get_top_columns(df_processed, [col for col in df_processed.columns if col.startswith('transp_')],
                                     top_n)
...
with st.expander("🌌 Dimensional Reduction with UMAP for Incident Analysis ✨", expanded=True):
    st.markdown(
    ...
    if st.button('Generate UMAP Visualization'):
        ...
        df_embedding['Category'] = df_filtered_incidents[top_k_columns].idxmax(axis=1).apply(
